...
#gets the static meshes in the unreal scene
    ...
    #NNOTE 6.0 all new
    def simSetSpecialValue(self, value, index, vehicle_name: str = ''):
        """
        Set a special runtime parameter on the target vehicle (e.g., DesiredYaw1/2).

        Args:
            value (float): The value to set.
            index (int): Special value index (e.g., 1 -> DesiredYaw1, 2 -> DesiredYaw2).
            vehicle_name (str, optional): Name of the target vehicle. Defaults to '' (active/default vehicle).

        Returns:
            bool: True if the value was applied successfully, otherwise False (depending on server implementation).
        """
        # Prefer the 3-argument RPC (newer servers). Fallback to 2-argument RPC for older servers.
        return self.client.call('simSetSpecialValue', float(value), int(index), vehicle_name)
